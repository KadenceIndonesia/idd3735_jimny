...
exports.getSourceOfInformationContent = async function (req, res) {
  ...
  for (let i = 0; i < result_a.length; i++) {
    for (let x = 0; x < result_a[i].step_1.length; x++) {
      var findIndexStep1 = await findObj(
        attrB1,
        "code",
        `${result_a[i].step_1[x]}`
      );
      var getCodeStep1 = attrB1[findIndexStep1];
      if (findIndexStep1 != -1) {
        var findArr = await findObj(data, "code", `${getCodeStep1.code}`);
        if (findArr == -1) {
          data.push({
            code: getCodeStep1.code,
            indexLabel: getCodeStep1.label,
            label: 0,
            y: 0,
            value: 1,
          });
  ...
  var dataSize = result_a.length;

  for (let i = 0; i < data.length; i++) {
    data[i].y = (data[i].value / dataSize) * 100;
    data[i].label = ((data[i].value / dataSize) * 100).toFixed(2);
  }
  ...
  if (checkParent != -1) {
    ...
    var dataB12018 = await dataFilterByBreak(
      "idd37352018",
      "B12018",
      "A7",
      break2,
    ...
    var dataB2a2018 = await dataFilterByBreak(
      "idd37352018",
      "B2a2018",
      "A7",
      break2,
  ...
  res.send([data, dataSize, result2018, dataLength2018]);
};
...
exports.getAidaContent = async function (req, res) {
  ...
  if (checkParent != -1) {
    ...
    var dataB1 = await dataFilterByBreak(
      pid,
      step[0],
      "A7",
      break2,
    ...
    var dataB2a = await dataFilterByBreak(
      pid,
      step[1],
      "A7",
      break2,
    ...
    var dataB5 = await dataFilterByBreak(
      pid,
      step[2],
      "A7",
      break2,
